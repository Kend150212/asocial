...
function buildGuide(domain: string) {
    ...
    client_max_body_size 100M;
            ...
            steps: [
                {
                    ...
                    callbackUrls: [
                        ...
                        { label: 'Threads OAuth', path: '/api/oauth/threads/callback' },
        ...
        },
        {
            ...
            steps: [
                {
                    title: 'Threads API App Setup',
                    description: 'Create a dedicated Meta App with the Threads API product to publish content on Threads.',
                    details: [
                        '1. Go to developers.facebook.com → My Apps → Create App',
                        '2. Select "Other" → Business type → Next → Create App',
                        '3. In App Dashboard → left sidebar → Add Product → find "Threads API" → Set Up',
                        '4. Threads API Settings → add Redirect URI: {YOUR_DOMAIN}/api/oauth/threads/callback',
                        '5. App Settings → Basic → copy App ID and App Secret',
                        '6. Admin → Integrations → Threads → paste App ID + App Secret → Save',
                        '7. App Roles → Roles → Add Testers (Development mode — only testers can connect)',
                        '8. Required scopes: threads_basic, threads_content_publish, threads_manage_insights, threads_manage_replies, threads_read_replies',
                        '9. Submit App Review to unlock all scopes for production use',
                        'ℹ️ Uses a SEPARATE Meta App from Facebook/Instagram',
                        'ℹ️ Access tokens are long-lived (60 days) and refreshed automatically',
                    ],
                    callbackUrls: [
                        { label: 'Threads OAuth Redirect URI', path: '/api/oauth/threads/callback' },
                    ],
                    links: [{ label: 'Meta Developer Portal', href: 'https://developers.facebook.com/apps/' }],
                    adminPath: '/admin/integrations',
                },
            ],
        },
